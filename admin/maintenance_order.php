...
while(list($main_order_id,$name,$user_id,$date,$status)=mysqli_fetch_array($result2))
		...
		<h5 class="card-title"> Finished Orders</h5>   <tr>
				...
				<tbody>';
			$query2 = "SELECT `main_order_id`,`name`,`user_id`, `date`,`status` FROM `maintenance_order` NATURAL JOIN `user` where maintenance_order.user_id=user.phone_number and `status`='finished'";
$result2 = mysqli_query($link, $query2);
